...
export class PokemonDetails implements OnInit {
  ...
  loadFormDetails(form: PokemonForm) {
    this.pokemonService.getPokemon(form.pokemonId.toString()).subscribe(data => {
  ...
  getSpriteGenerations(): string[] {
    ...
    return generationOrder.filter(gen =>this.formPokemon.sprites.versions[gen]);
  }
  ...
  getSpriteGames(generation:string): string[] {
    ...
    const available =Object.keys(this.formPokemon.sprites.versions[generation]);
    if(gameOrder[generation]) {return gameOrder[generation].filter((game:string)=> available.includes(game));}
    return available;
  }

  getSpriteList(generation:string, game:string) {
    const sprites = this.formPokemon ?.sprites ?.versions[generation]?.[game];
    if(!sprites) return [];
    ...
    return allowedSprites.filter(key => sprites[key])
        .filter(key => {
            const url = sprites[key];
            if(seen.has(url)) {return false;}
            seen.add(url);
            return true;
        }).map(key => ({name:key, url:sprites[key]}));
  }
  ...
  getModernSprites() {
    if (!this.formPokemon?.sprites?.other) {return [];}
    const sprites = this.formPokemon.sprites.other;
    const results:any[] = [];
    if (sprites.home) {
        if (sprites.home.front_default) {results.push({name: "Pokémon HOME", url: sprites.home.front_default});}
        if (sprites.home.front_shiny) {results.push({name: "Pokémon HOME Shiny", url: sprites.home.front_shiny});}
        if (sprites.home.front_female) {results.push({name: "Pokémon HOME Female", url: sprites.home.front_female});}
        if (sprites.home.front_shiny_female) {results.push({name: "Pokémon HOME Female Shiny", url: sprites.home.front_shiny_female});} 
    }
    if (sprites.showdown) {
        if (sprites.showdown.front_default) {results.push({name:"Animated", url:sprites.showdown.front_default});}
        if (sprites.showdown.front_shiny) {results.push({name:"Animated Shiny", url:sprites.showdown.front_shiny}); }
    }
    if (sprites['official-artwork']) {
        if (sprites['official-artwork'].front_default) {results.push({name:"Official Artwork", url:sprites['official-artwork'].front_default});}
        if (sprites['official-artwork'].front_shiny) {results.push({name:"Official Artwork Shiny", url:sprites['official-artwork'].front_shiny});}
    }
    return results;
  }

  getModernSpritesByGame(game: string) {
    if (!this.formPokemon?.sprites) {return [];}
    const sprites = this.formPokemon.sprites;
    const modernSprites: any[] = [];
    if (game === 'pokemon-home') {
        if (sprites.other?.home?.front_default) {modernSprites.push({name: 'Home', url: sprites.other.home.front_default});}
        if (sprites.other?.home?.front_shiny) {modernSprites.push({name: 'Home Shiny', url: sprites.other.home.front_shiny});}
    }
    if (game === 'scarlet-violet') {
        const sv = sprites.versions?.['generation-ix']?.['scarlet-violet'];
        if (sv?.front_default) {modernSprites.push({name: 'Scarlet Violet', url: sv.front_default});}
        if (sv?.front_shiny) {modernSprites.push({name: 'Scarlet Violet Shiny',url: sv.front_shiny});}
    }
    if (game === 'sword-shield') {
        const swsh = sprites.versions?.['generation-viii']?.['icons'];
        if (swsh?.front_default) {modernSprites.push({name: 'Sword Shield Icon', url: swsh.front_default});}
    }
    return modernSprites;
    }

    loadMoveDetails() {
    if (!this.formPokemon?.moves) {return;}
    const requests: Observable<any>[] = this.formPokemon.moves.map((move: any) =>this.pokemonService.getMove(move.move.name));
    forkJoin(requests).subscribe((moves:any[]) => {
        const allMoves = moves.map((move:any,index:number)=> {
            const methods =this.formPokemon.moves[index].version_group_details;
            return {
                id: move.id,
                name: move.name,
                type: move.type.name,
                ...
                pp: move.pp,
                effect:move.effect_entries?.find((e:any)=>e.language.name === 'en')?.short_effect ?? '',
                methods,
                level:methods.find((m:any) => m.move_learn_method.name === 'level-up')?.level_learned_at ?? null
            };
        });
        this.moveGroups = {
  ...
  getLevel(move:any):number {
    const level = move.methods.find((m:any)=>m.move_learn_method.name === 'level-up')?.level_learned_at;
    return level ?? 999;
  }

    groupMoves() {
        this.moveGroups = {levelUp: [], tm: [], tutor: [], egg: [], evolution: []};
        this.moveDetails.forEach(move => {
            move.methods.forEach((method:any) => {
                const entry = {...move, level: method.level_learned_at, method: method.move_learn_method.name};
        ...
        this.moveGroups.levelUp.sort((a,b) => a.level - b.level);
    }
